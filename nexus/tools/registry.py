...
# 全局 ToolRegistry 单例
# ---------------------------------------------------------------------------
# 设计意图：进程级单例，在 API 进程和 ARQ Worker 进程之间共享 Tool 定义。
# 线程安全注意事项：
#   - 此单例仅存储无状态工具定义（函数引用、元数据），不存储租户特定数据
#   - 有状态资源（连接池、缓存、API Keys）在各 Tool 执行时按需创建
#   - 全局初始化在 app startup 阶段完成，运行时仅读取
#   - 如需测试隔离，使用 set_tool_registry() 替换
# ---------------------------------------------------------------------------
